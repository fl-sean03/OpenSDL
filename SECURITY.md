...
| No credentials in Git | **Partly implemented.** A manifest names a credential with `${env:NAME}`, resolved from the environment before validation and refused when it resolves to nothing, when it appears in a mapping key, or anywhere under `spec.policy`. A resolved value is written back as its reference by `dump_manifest` and in the context pack, and `doctor` redacts the database URL's userinfo and credential-bearing query parameters. Nothing stops a credential being typed into the manifest directly: there is no `SecretStr`, no provider beyond `env:`, and no secret scanning in `make lint` or CI. An adapter receives the resolved credential, and anything it echoes — from `health()`, an exception, or capability metadata — is printed unfiltered. |
