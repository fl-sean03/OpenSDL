from opensdl_provenance import (
...
def test_transitive_impact() -> None:
    graph = PropagationGraph(
        PropagationDefinition(
            ...
            edges=[
                PropagationEdge(
                    source="schema", target="adapter", reason="adapters implement contracts"
                ),
                PropagationEdge(
                    source="adapter", target="docs", reason="integration docs describe adapters"
                ),
            ],
        )
    )
...
def test_repository_graph_covers_domain_pack_changes() -> None:
    ...
    impact = graph.affected(["domain-packs/materials/src/opensdl_domain_materials/models.py"])
    assert {"domain-packs", "examples", "documentation"} <= set(impact.affected_nodes)
