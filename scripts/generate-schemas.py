...
def main() -> None:
    ...
    if not args.check:
        generated = generate_json_schemas(DESTINATION)
        print(f"generated {len(generated)} schemas in {DESTINATION}")
        return
    with tempfile.TemporaryDirectory() as temporary:
        candidate = Path(temporary)
        generate_json_schemas(candidate)
        ...
        changed = [name for name in expected if not filecmp.cmp(DESTINATION / name, candidate / name, shallow=False)]
        if changed:
